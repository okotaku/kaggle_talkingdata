...
#----------------------------------------------------------------------------------------------------------------
    ...
    del test_df
    sub.to_csv('bai-lgbsub.csv.gz', float_format='%.8f', index=False, compression='gzip')
    del sub
    gc.collect()
    
    pred_val = bst.predict(val_df[predictors],num_iteration=best_iteration)
    del val_df
    gc.collect()
    pred_df = pd.DataFrame()
    pred_df['is_attributed']  = pred_val
    pred_df.to_csv('bai-lgbval.csv.gz', float_format='%.8f', index=False,compression='gzip')
    print("All done...")
